...
func title_show(id_ui: String, id: String):
	...
	for i in range(len(GameConstants.DATA_TOWER[int(id) - 1].text)):
		if GameConstants.DATA_TOWER[int(id) - 1]["parametr_" + str(i + 1)] is Dictionary:
			node_mouse_entered.list_node[i].get_node("HValue/Value").text = str(GameConstants.DATA_TOWER[int(id) - 1]["parametr_" + str(i + 1)][int(DataManager.tower_data["Turret_" + str(i + 1) + "T1"]["level"])][0])
		else:
			node_mouse_entered.list_node[i].get_node("HValue/Value").text = str(GameConstants.DATA_TOWER[int(id) - 1]["parametr_" + str(i + 1)][0])
