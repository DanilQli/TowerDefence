...
func setup(i, type):
	if type < 3:
		get_node("HBoxContainer/VBoxContainer/RichTextLabel").text = tr("KEY_TASK_" + str(int(TasksManager.list_tasks_you[i][0]))) + str(int(TasksManager.list_tasks_you[i][1]))
		...
		get_node("HBoxContainer/VboxIcon/NinePatchRect").texture = DataManager.TYPE_ITEMS[int(TasksManager.list_tasks_you[i][0])][2]
		if type == 1:
		...
		get_node("HBoxContainer/VBoxContainer/RichTextLabel").text = tr("KEY_TASK_" + str(int(TasksManager.daily_task_career[i][2]))) + str(int(TasksManager.daily_task_career_you[i][2]))
